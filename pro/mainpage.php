...
<head>
    ...
    <style>
        ...
        .main { height: 600px; width: 400px; border-radius: 10px; background-color: transparent; box-shadow: 0 4px 8px rgba(0, 0, 0, 0.1); display: flex; flex-direction: column; align-items: center; padding: 20px; position: relative; }
        ...
        button[type="submit"], .edit-button { padding: 10px; border: none; border-radius: 25px; cursor: pointer; background-color:rgba(0, 0, 0, 0.562); color: white; font-size: 1em; transition: background-color 0.3s ease; }
            ...
            <div class="clickbuttons">
                ...
                if ($result->num_rows > 0) {
                    while ($row = $result->fetch_assoc()) {
                        echo "<div>
                                <button class='clickbutton' onclick=\"location.href='attendance.php?subject_id={$row['id']}'\">{$row['subject_name']} ({$row['total_hours']} hrs)</button>
                                <button class='edit-button' onclick=\"location.href='edit_subject.php?subject_id={$row['id']}&subject_name={$row['subject_name']}&total_hours={$row['total_hours']}'\">Edit</button>
                              </div>";
                    }
                } else {
